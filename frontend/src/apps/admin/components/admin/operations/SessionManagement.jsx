                    ...
                    sessions.map((session, index) => (
                        <div key={session.id || index} style={{
                            ...
                            <div style={{ display: 'flex', justifyBetween: 'space-between', alignItems: 'flex-start', marginBottom: '20px', justifyContent: 'space-between' }}>
                                ...
                                <span style={{
                                    padding: '4px 10px',
                                    background: session.status === 'reserved' ? '#fef9c3' : '#eff6ff',
                                    color: session.status === 'reserved' ? '#854d0e' : '#3b82f6',
                                    borderRadius: '8px', fontSize: '11px', fontWeight: '800',
                                    textTransform: 'uppercase', letterSpacing: '0.05em',
                                    border: session.status === 'reserved' ? '1px solid #fef08a' : 'none',
                                    animation: session.status === 'active' ? 'pulse 2s infinite' : 'none'
                                }}>
                                    {session.status === 'reserved' ? '● RESERVED' : 'Live'}
                                </span>
                            </div>
