                                ...
                                <tr key={log.id || index} style={{
                                    ...
                                    <td style={{ padding: '20px 32px' }}>
                                        <span style={{
                                            ...
                                            backgroundColor: log.status === 'reserved' ? '#fef9c3' : (log.is_active ? '#dcfce7' : '#f1f5f9'),
                                            color: log.status === 'reserved' ? '#854d0e' : (log.is_active ? '#15803d' : '#475569'),
                                            boxShadow: (log.is_active || log.status === 'reserved') ? '0 4px 6px -1px rgba(0, 0, 0, 0.05)' : 'none'
                                        }}>
                                            <div style={{
                                                width: '6px',
                                                height: '6px',
                                                borderRadius: '50%',
                                                backgroundColor: log.status === 'reserved' ? '#f59e0b' : (log.is_active ? '#22c55e' : '#64748b'),
                                                animation: (log.is_active && log.status !== 'reserved') ? 'pulse 2s infinite' : 'none'
                                            }}></div>
                                            {log.status === 'reserved' ? 'Reserved' : (log.is_active ? 'Active' : 'Completed')}
                                        </span>
                                    </td>
                                </tr>
                            ))
