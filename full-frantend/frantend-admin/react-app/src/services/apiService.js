...
class ApiService {
  ...
  async getStaffSalaries(month, year) {
    const response = await apiClient.get('/api/staff/salaries/', { params: { month, year } });
    return response.data;
  }

  async updateStaffSalary(salaryId, salaryData) {
    const response = await apiClient.put(`/api/staff/salaries/${salaryId}/`, salaryData);
    return response.data;
  }

  async payStaffSalary(paymentData) {
    const response = await apiClient.post('/api/staff/salaries/', paymentData);
    return response.data;
  }
