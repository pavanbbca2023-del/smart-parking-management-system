...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'username', 'email', 'role', 'salary', 'position', 'phone', 'created_at', 'assigned_zones', 'password', 'plain_password')
        extra_kwargs = {
...
class ParkingSessionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        # Mask guest_mobile and guest_email for privacy if not staff/admin
        is_staff = False
        ...
        if not is_staff:
            if representation.get('guest_mobile'):
                mobile = representation['guest_mobile']
                if len(mobile) >= 4:
                    representation['guest_mobile'] = f"{mobile[:2]}XXXXXX{mobile[-2:]}"
            
            if representation.get('guest_email'):
                email = representation['guest_email']
                if '@' in email:
                    name, domain = email.split('@')
                    masked_name = name[0] + 'X' * (len(name) - 1) if len(name) > 1 else 'X'
                    representation['guest_email'] = f"{masked_name}@{domain}"
