...
def check_revenue():
    ...
    for s in paid_sessions:
        print(f"Vehicle: {s.vehicle_number}, Amount: {s.amount_paid}, Date: {s.entry_time}")
        
    total = paid_sessions.aggregate(total=Sum('amount_paid'))['total']
    ...
    today_total = today_paid.aggregate(total=Sum('amount_paid'))['total']
