...
from .sms_service import Fast2SMSService
import logging
    ...
    def process_cancellation(session, reason, user=None, cancellation_type='user_initiated'):
        ...
        try:
            ...
            if cancellation_type == 'user_initiated':
                Fast2SMSService.send_cancellation_confirmation(session)
            else:
                Fast2SMSService.send_auto_cancellation_notice(session)
    ...
    def process_extension(session, hours, user=None):
        ...
        try:
            ...
            if success:
                ...
                # Send SMS confirmation
                Fast2SMSService.send_extension_confirmation(session, hours)
    ...
    def send_expiry_warnings():
        ...
        for session in expiring_sessions:
            if session.is_expiring_soon():
                # Send warning SMS
                success, response = Fast2SMSService.send_cancellation_warning(session)
