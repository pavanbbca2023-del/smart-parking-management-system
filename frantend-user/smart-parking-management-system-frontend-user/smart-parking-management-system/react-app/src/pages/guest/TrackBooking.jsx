...
const TrackBooking = ({ onNavigate }) => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (latestSession.status.toLowerCase() === 'active') {
                const bookingData = {
                    ...
                    exitTime: latestSession.exit_time ? new Date(latestSession.exit_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'N/A',
                    totalAmount: latestSession.total_amount_paid,
                    amountPaid: latestSession.initial_amount_paid,
                    remainingBalance: latestSession.total_amount_paid - latestSession.initial_amount_paid,
                ...
                setError('Found a record, but it is neither active nor completed.');
            }
        } catch (err) {
            console.error(err);
